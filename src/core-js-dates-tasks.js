...
function getWeekNumberByDate(date) {
  const targetDate = new Date(date.getFullYear(), 0, 1);
  let weekNumber = 0;

  if (targetDate.getDay() > 1) {
    targetDate.setDate(7 - targetDate.getDay() + 2);
    weekNumber += 1;
...
function setToNextFriday(date) {
  const dayOfWeek = date.getDay();
  const daysUntilFriday = dayOfWeek === 5 ? 7 : 5 - dayOfWeek;
  date.setDate(date.getDate() + daysUntilFriday);
  return date;
}

function getNextFridayThe13th(date) {
  setToNextFriday(date);
